...
const HomeComponent = ({ latestWorkflowData, chartData, testHistory, passPercentage }) => {

    const getDuration = (run) => {
        let startTime = moment(run.test_run.started, "HH:mm:ss a"), endTime = moment(run.test_run.completed, "HH:mm:ss a");        
                ...
                <div id="main-home">
                        ...
                        <div className="latest">
                            ...
                            <div style={{ display: 'flex', flexDirection: 'row' }}>
                                <div className="run-info">
                                    <div style={{ fontWeight: 600, marginLeft: 20 }}>{latestWorkflowData.workflow_run.title}</div>
                                    <div className='subTitle'>{moment(latestWorkflowData.workflow_run.created).format("MMM. Do YYYY")}</div>
                                    <div className='sub'>{latestWorkflowData.workflow_run.actor_name}</div>
                                    <div className="count">
                                        <div style={{ fontWeight: 'bolder', fontSize: 40 }}>{latestWorkflowData.test_runs.length}</div>
                                        <div>test cases</div>
                                    </div>
                                </div>
                                <div className="chart">
                        ...
                        <div className="test-list">
                            ...
                            </div>
                            {
                                latestWorkflowData.test_runs.map((run) => {
                                    return (
                                        <div style={{ margin: '2px 0', padding: '5px', cursor: 'pointer', borderBottom: '1px solid', backgroundColor: `${statuses[run.test_run.conclusion]}` }} key={run.test_run.id}>{run.test_run.name} - {getDuration(run)} secs</div>
                                    )
                                })
                            }
                        </div>
                    ...
                    <div id="right">
                        <div className="trends">
                            <h3>Test Trends</h3>
                            <LineChart isAnimationActive={true} width={800} height={300} data={chartData}>
                                        ...
                                        return (
                                            <div className='history-row' key={run.test_commit}>
                                                <div className='test-name'>
                                                    ...
                                                    <div className='sub'>{moment(run.test_date).format("MMM. Do YYYY")}</div>
                                                </div>    
